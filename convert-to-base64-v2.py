...
# Map URLs to local files - ALL ASSETS
url_map = {
    # Fonts
...
for url, filepath in url_map.items():
    ...
    if path.exists():
        try:
            ...
            # Replace URL-encoded version too
            url_encoded = url.replace('%20', ' ').replace('%2520', ' ')
            html = html.replace(url, data_uri)
            if url != url_encoded:
                html = html.replace(url_encoded, data_uri)
            converted += 1
...
print(f"\nConverted {converted}/{len(url_map)} assets")
...
# Keep external docs.rail.io link but make other pages anchor links
...
print(f"\n✓ Saved to {output_file} ({file_size / (1024*1024):.1f} MB)")

# Check for remaining external URLs (excluding docs.rail.io which is external docs)
remaining = re.findall(r'https://cdn\.prod\.website-files\.com/[^"\'\s)]+', html)
unique_remaining = sorted(set(remaining))

if unique_remaining:
    print(f"\n⚠ WARNING: {len(unique_remaining)} unique external CDN URLs still found:")
    for url in unique_remaining[:15]:
        print(f"  - {url}")
    if len(unique_remaining) > 15:
        print(f"  ... and {len(unique_remaining) - 15} more")
else:
    print("\n✅ SUCCESS! NO external CDN URLs found!")
    print("✅ File is 100% localized with 0 external dependencies")
